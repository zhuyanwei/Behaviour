...
ForegroundExtraction::ForegroundExtraction()
{

}

int ForegroundExtraction::mainly()
{
    /*
    const string absPath = "C:\\Fg-detection-dynamic-background\\"; //"C:/Users/Agata/Desktop/highway";
    string path = absPath + "\\Fg-detection-dynamic-background\\";
    const string datasets[3] = {"dynamicBackground", "baseline", 'cameraJitter'};
    const string sampleNames[3][6] = {{"boats","canoe","fall","fountain01","fountain02","overpass"},{"airport","highway", "office", "pedestrians", "PETS2006"},{"badminton","boulevard","sidewalk","traffic"}};
    int mode = 0;
    string basicPath = absPath + datasets[mode] + "\\";
    for (int i = 0; i < 6; i++) {
...
//        FTSG(path);
    ...
    mode = 1;
    basicPath = absPath + datasets[mode] + "\\" + datasets[mode] + "\\";
    for (int i = 0; i < 4; i++) {
        path = basicPath + sampleNames[mode][i] + "\\input\\";
        FTSG(path);
        CwisarDH(path);
...
*/
    string inputPath = absPath + "\\input\\";
//    FTSG(inputPath);
//    CwisarDH(inputPath);
...
int ForegroundExtraction::countDir(const char *path)
{
    DIR *dp;
    int i = 0;
    dp = opendir (path);
    ...
    if (dp != NULL)
    {
        ...
        perror ("Couldn't open the directory222");

    return i;
}

void ForegroundExtraction::drawOptFlowMap(const Mat& flow, Mat& cflowmap, int step,double, const Scalar& color)
{
...
void ForegroundExtraction::calculateAverage(vector<Mat> in[3], vector<Mat> average)
{
    Point2f avg;
...
void ForegroundExtraction::calculateVariance(vector<Mat> in[3], vector<Mat> average, Mat tensor)
{
    Point2f var;
    int sampleCount = in[0].size();
    int rows = in[0][0].rows;
...
void ForegroundExtraction::calculateAddress(Vec3b ptr, ushort map[192][3], ushort address[36])
{
    ushort label;
    for (int i = 0; i < 36; i++) {
        address[i] = 0;
...
int ForegroundExtraction::GMM(string path)
{
    ...
    string foregroundPath = absPath + "\\foreground\\";
    string maskPath = absPath + "\\mask\\";
...
//    namedWindow("Split Gaussian", CV_WINDOW_AUTOSIZE); //create a window with the name "Flux Tensor"
    namedWindow("mask", CV_WINDOW_AUTOSIZE); //create a window with the name "Flux Tensor"
    namedWindow("foreground", CV_WINDOW_AUTOSIZE); //create a window with the name "Flux Tensor"
...
//        imshow("Split Gaussian", bgMaskOpen);
        ...
        Mat foreground;// = srcImage(Rect(200,250,frame.cols,frame.rows));
        frame.copyTo(foreground,bgMaskOpen);
        imshow("input", frame);
        imshow("mask", bgMaskOpen);
        imshow("foreground",foreground);

        imwrite(foregroundPath+filename,foreground);
        imwrite(maskPath+filename,bgMaskOpen);
...
//    destroyWindow("Split Gaussian"); //destroy the window with the name, "Split Gaussian"
    ...
    destroyWindow("mask"); //destroy the window with the name, "Split Gaussian"
    destroyWindow("foreground"); //destroy the window with the name, "Split Gaussian"
}

int ForegroundExtraction::ViBE(string path)
{
    int sampleCount = countDir(path.c_str()) - 2;
    char filename[12];
    Mat frame;
    ...
    string foregroundPath = absPath + "\\foreground\\";
    string maskPath = absPath + "\\mask\\";
    if (access(foregroundPath.c_str(), 0) == -1)
    {
        mkdir(foregroundPath.c_str());
    }
    if (access(maskPath.c_str(), 0) == -1)
    {
        mkdir(maskPath.c_str());
    }
    namedWindow("input", CV_WINDOW_AUTOSIZE); //create a window with the name "Flux Tensor"
    namedWindow("mask", CV_WINDOW_AUTOSIZE); //create a window with the name "Flux Tensor"
    namedWindow("foreground", CV_WINDOW_AUTOSIZE); //create a window with the name "Flux Tensor"
    for (int i = OMITTED_FRAMES + 1; i < sampleCount; i++) {
        {
            ...
            Mat foreground;// = srcImage(Rect(200,250,frame.cols,frame.rows));
            frame.copyTo(foreground,fg);
            imshow("input", frame);
            imshow("mask", fg);
            imshow("foreground",foreground);
            imwrite(foregroundPath+filename,foreground);
            imwrite(maskPath+filename,fg);
            waitKey(1);
        }
        cout << i << endl;
    }
...
int ForegroundExtraction::PBASfun(string path)
{
//    int sampleCount = countDir(path.c_str()) - 2;
    int sampleCount = 2;
    char filename[12];
    ...
    {
        {
            ...
            Mat foreground;// = srcImage(Rect(200,250,frame.cols,frame.rows));
            frame.copyTo(foreground,pbasResult);
//            imshow("input", frame);
//            imshow("mask", pbasResult);
//            imshow("foreground",foreground);

            imwrite(foregroundPath+filename,foreground);
            imwrite(maskPath+filename,pbasResult);
...
int ForegroundExtraction::FTSG(string path)
{
    int sampleCount = countDir(path.c_str()) - 2;
    char filename[12];
    ...
    for (int i = OMITTED_FRAMES + 1; i < BUFFER_SIZE + OMITTED_FRAMES; i++)
    { //loop over frames
...
int ForegroundExtraction::CwisarDH(string path)
{
    int sampleCount = countDir(path.c_str()) - 2;
    char filename[12];
    Mat frame;
    ...
    for (int i = OMITTED_FRAMES; i < BUFFER_SIZE + OMITTED_FRAMES; i++)
    { // initialization
        ushort address[36];
        ushort ind, pos;
